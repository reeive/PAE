...
def get_model_core(model: nn.Module) -> nn.Module:
...
@torch.no_grad()
def _make_random_like(p: torch.Tensor) -> torch.Tensor:
    return torch.randn_like(p).to(dtype=p.dtype)


@torch.no_grad()
def _filterwise_norm_scale(direction: torch.Tensor, weight: torch.Tensor, layer: nn.Module) -> torch.Tensor:
    d, w = direction, weight
    if isinstance(layer, nn.Conv2d) or isinstance(layer, nn.Linear):
        out_c = w.shape[0]
        w_v = w.reshape(out_c, -1)
        d_v = d.reshape(out_c, -1)
        w_norm = w_v.norm(dim=1) + 1e-12
        d_norm = d_v.norm(dim=1) + 1e-12
        scale = (w_norm / d_norm).unsqueeze(1)
        d = (d_v * scale).reshape_as(weight)
    else:
        w_norm = w.norm() + 1e-12
        d_norm = d.norm() + 1e-12
        d = d * (w_norm / d_norm)
...
def _should_ignore(name: str, module: nn.Module, ignore_keywords: Tuple[str, ...]) -> bool:
    if any(k in name.lower() for k in ignore_keywords):
        return True
    if isinstance(module, (nn.BatchNorm1d, nn.BatchNorm2d, nn.BatchNorm3d, nn.SyncBatchNorm, nn.LayerNorm)):
        return True
    return False


@torch.no_grad()
def build_filter_normalized_directions(
        model: nn.Module,
        ignore_keywords: Tuple[str, ...] = ("bias", "bn", "running_mean", "running_var", "num_batches_tracked", "norm"),
    ...
    for (name, p) in core.named_parameters():
        ...
        if (not p.requires_grad) or _should_ignore(name, parent, ignore_keywords):
            dir_x[name] = torch.zeros_like(p)
            dir_y[name] = torch.zeros_like(p)
            continue
        rx, ry = _make_random_like(p), _make_random_like(p)
        rx = _filterwise_norm_scale(rx, p, parent)
        ry = _filterwise_norm_scale(ry, p, parent)
        dir_x[name] = rx
        dir_y[name] = ry
    return dir_x, dir_y
...
def displaced_params(model: nn.Module, base_state: Dict[str, torch.Tensor],
                     dx: Dict[str, torch.Tensor], dy: Optional[Dict[str, torch.Tensor]],
                     alpha: float, beta: float = 0.0):
    core = get_model_core(model)
    current = {k: v.detach().clone() for k, v in core.state_dict().items() if v.dtype.is_floating_point}
    new_state = {}
    for k, v in base_state.items():
        if k in dx:
            ...
            new_state[k] = v
    core.load_state_dict({**current, **new_state}, strict=False)
    try:
        yield
    finally:
