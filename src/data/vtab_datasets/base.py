...
import os
os.environ['TFDS_DISABLE_DOWNLOAD'] = '1'


class Vtab1k(tfds.core.GeneratorBasedBuilder):
  VERSION = tfds.core.Version("1.0.0")

  def _info(self):
    return tfds.core.DatasetInfo(
      builder=self,
      description="自定义的 VTAB-1k 数据集，本地数据",
  ...
  def _split_generators(self, dl_manager):
    # 假设你数据目录下有 train、val、test 三个子文件夹
    data_dir = "/data/lipei/VPT/vtab-1k"
    return [
  ...
  def _generate_examples(self, data_dir):
    # 这里假设你每个 split 下存储的是 TFRecord 文件，你需要根据实际情况解析
    record_files = glob.glob(os.path.join(data_dir, "*.tfrecord"))
    if not record_files:
      raise ValueError(f"在 {data_dir} 中未发现 TFRecord 文件")
    ...
    for record_file in record_files:
      ...
      for serialized_example in ds:
        parsed_example = _parse_record(serialized_example)
        # 这里生成唯一 key，可以根据实际情况更换标识
        key = f"{os.path.basename(record_file)}_{hash(serialized_example.numpy())}"
        yield key, parsed_example



builder = tfds.builder("dmlab", data_dir="/data/lipei/VPT/vtab-1k")
# 不调用 download_and_prepare()，因为数据已经在本地
